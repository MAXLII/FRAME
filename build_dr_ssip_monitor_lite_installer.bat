...
set "APP_NAME=DR_SSIP_Monitor"
set "PUBLISHER=LWX"
set "ISS_FILE=%~dp0installer\dr_ssip_monitor_lite_installer.iss"
set "DIST_APP_DIR=%~dp0dist\frame"
set "BRAND_FILE=%~dp0installer\dr_ssip_monitor_lite_brand.txt"
set "ISCC_EXE="
...
for /f "tokens=1-3 delims=." %%A in ("%BASE_APP_VERSION%") do set "APP_VERSION=2.%%B.%%C"
if not defined APP_VERSION (
    echo [ERROR] Failed to derive lite installer version from "%BASE_APP_VERSION%"
    exit /b 1
)
...
echo [INFO] Building FRAME application bundle for %APP_NAME% lite installer...
...
echo [INFO] Building %APP_NAME% lite installer package...
...
echo.
echo [OK] Lite installer build complete: %~dp0dist\installer\DR_SSIP_Monitor-Setup-%APP_VERSION%.exe

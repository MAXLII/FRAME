...
class TraceTab(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        ttk.Entry(control, textvariable=self.target_addr_var, width=4, style="Trace.TEntry").grid(row=0, column=1, sticky="ew", padx=(6, 10), pady=(0, 8))
        ttk.Label(control, text="Dyn", style="Trace.PanelMuted.TLabel").grid(row=0, column=2, sticky="w", pady=(0, 8))
        ttk.Entry(control, textvariable=self.dynamic_addr_var, width=4, style="Trace.TEntry").grid(row=0, column=3, sticky="ew", padx=(6, 10), pady=(0, 8))
        ttk.Label(control, text="行号", style="Trace.PanelMuted.TLabel").grid(row=0, column=4, sticky="w", pady=(0, 8))
        ttk.Entry(control, textvariable=self.search_line_var, style="Trace.TEntry", width=8).grid(row=0, column=5, sticky="ew", padx=(6, 0), pady=(0, 8))
        self.toggle_button = ttk.Button(control, text="开始上报", command=self.on_toggle_trace, style="Trace.Accent.TButton")
        self.toggle_button.grid(row=1, column=0, columnspan=6, sticky="ew", pady=(4, 10))
        self.search_line_var.trace_add("write", lambda *_args: self._refresh_highlights())
        ttk.Label(control, text="状态", style="Trace.PanelMuted.TLabel").grid(row=2, column=0, sticky="w")
        ttk.Label(control, textvariable=self.status_var, style="Trace.Status.TLabel").grid(row=2, column=1, columnspan=5, sticky="ew")

        stats = ttk.Frame(side, style="Trace.Panel.TFrame")
        stats.grid(row=4, column=0, sticky="new", pady=(18, 0))

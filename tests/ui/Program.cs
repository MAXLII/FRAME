            ...
            try
            {
                ...
                foreach(DictionaryEntry entry in pages)
                {
                    ...
                    if(group=="wave"){
                        ...
                        if(charts.Any(chart=>chart.Plot.GetPlottables<ScottPlot.Plottables.VerticalLine>().Single().X!=4)||charts[0].Plot.GetPlottables<ScottPlot.Plottables.HorizontalLine>().Any()||charts[1].Plot.GetPlottables<ScottPlot.Plottables.HorizontalLine>().Single().Y!=20)throw new Exception("Shared vertical cursor and active-pane-only horizontal cursor failed");
                        show.Invoke(window,[entry.Value,new JsonObject{["records"]=preview.DeepClone()}]);

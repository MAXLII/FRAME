...
- 为上位机与设备之间建立统一、稳定、可扩展的串口、CAN 和 Ethernet TCP 通信机制
...
- 使用本地缓存保存实时上报点
- 实时跟随状态按当前预设或自定义时间跨度滚动到最新数据
- 横向移动或框选早期数据后进入历史查看，固定时间轴视口并继续缓存上报点
- 历史查看期间不因新数据重绘波形画布，最新值和实时保存继续更新
- 纵轴缩放与时间轴跟随状态相互独立
- 将实时查看与交互分析放在同一页面内
...
1. 在参数页勾选需要显示的参数
2. 进入参数波形页开始发波
3. 选择预设时间窗口，或选择“自定义”输入时间跨度
4. 在最新数据端缩放时，波形按缩放后的自定义跨度继续实时跟随
5. 横向移动或框选早期数据，固定当前历史视口
6. 点击“回到实时”后按当前视口跨度恢复最新数据跟随
7. 使用参考线、纵轴缩放和导出功能分析数据

#### 3.4.6 注意事项

- 本页是实时波形，不等同于 Scope 本地录波
- 历史查看只暂停波形画布随新数据刷新，不停止数据接收、最新值更新和实时保存
...
- 双击变量或结构体行时读取对应变量当前值
- 展开结构体、数组或指针时，根据已知类型和地址读取对应内存并生成子项
- 指针变量的展开地址来自 J-Link 读取到的内存值，不从 `Value` 列显示文本解析
- 指针显示可以带符号名和地址，例如 `section_com_0x30u_0x01u (0x200000F0)`，业务逻辑仍使用结构化指针值
- 字符串指针和字符数组在可读取范围内显示 ASCII 内容
- RAM 写入只能写 RAM 范围，Flash 和代码区域不允许通过变量写入入口修改
- 写入值由用户在 `Value` 列编辑并按 Enter 后提交，未按 Enter 时不写入
- 刷新按钮只刷新当前可见变量行，未展开结构体和数组不读取隐藏成员
...
| 日期 | 模块 | 类型 | 说明 |
| --- | --- | --- | --- |
| 2026-07-04 | J-Link 变量读取 | 更新 | 补充 J-Link 变量 Load、读取、展开、刷新、搜索、写入和显示文本边界说明 |
